#> pk_wast:iris/find_closest_surface/north/loop
#
# Determines if the ray hits the current surface
#
# @within pk_wast:iris/find_closest_surface/north/main
# @within pk_wast:iris/find_closest_surface/north/loop
...
execute store result score $y1 iris run data get storage pk_wast:iris.data List[-1][4]

# If the current position is already in the plane of the surface, check if it is in the surface
execute if score ${z} iris = $z0 iris if score ${x} iris >= $x0 iris if score ${x} iris <= $x1 iris if score ${y} iris >= $y0 iris if score ${y} iris <= $y1 iris run function pk_wast:iris/find_closest_surface/ray_already_in_surface

# If the current position is before the plane, check if it hits the surface and get the position of that intersection
execute if score ${z} iris < $z0 iris run function pk_wast:iris/find_closest_surface/north/get_distance
...
execute if score $surfaces iris matches 1.. run function pk_wast:iris/find_closest_surface/north/loop