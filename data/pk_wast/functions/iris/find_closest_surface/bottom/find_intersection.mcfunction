#> pk_wast:iris/find_closest_surface/bottom/find_intersection
#
# Get position of the intersection, and see if that position is within the current surface
#
# @within pk_wast:iris/find_closest_surface/bottom/get_distance
...
execute if score $x_intersection iris >= $x0 iris if score $x_intersection iris <= $x1 iris if score $z_intersection iris >= $z0 iris if score $z_intersection iris <= $z1 iris run function pk_wast:iris/find_closest_surface/ray_intersects_with_surface