#> pk_wast:iris/find_closest_surface/west/loop
#
# Determines if the ray hits the current surface
#
# @within pk_wast:iris/find_closest_surface/west/main
# @within pk_wast:iris/find_closest_surface/west/loop
...
execute if score ${x} iris = $x0 iris if score ${y} iris >= $y0 iris if score ${y} iris <= $y1 iris if score ${z} iris >= $z0 iris if score ${z} iris <= $z1 iris run function pk_wast:iris/find_closest_surface/ray_already_in_surface

# If the current position is before the plane, check if it hits the surface and get the position of that intersection
execute if score ${x} iris < $x0 iris run function pk_wast:iris/find_closest_surface/west/get_distance
...
execute if score $surfaces iris matches 1.. run function pk_wast:iris/find_closest_surface/west/loop